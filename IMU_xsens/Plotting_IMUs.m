...
for ii = 1:3
    [~,ix1] = min(abs(IMU(ii).stimem-0.05)); % Initial time for 1st pose
    [~,ix2] = min(abs(IMU(ii).stimem-0.1)); % Final time for 1st pose
    [~,ix3] = min(abs(IMU(ii).stimem-0.15)); % Initial time for 2nd pose
    [~,ix4] = min(abs(IMU(ii).stimem-0.2)); % Final time for 2nd pose
...
for ii = 1:length(IMU(1).stime)
    for jj = 1:3
        %JA(jj).Rgs = yprtoR(IMU(jj).yw(ii),IMU(jj).pt(ii),IMU(jj).rl(ii));
        JA(jj).Rgs = qtoR(IMU(jj).q.q0(ii),IMU(jj).q.q1(ii),IMU(jj).q.q2(ii),IMU(jj).q.q3(ii)); % Sensor to global matrix
